...
void test_stack_init_free(void) {
    ...
    stack_arr_init(&stack, test_N, sizeof(unsigned int));
    ...
    TEST_ASSERT_TRUE(stack_arr_is_empty(stack));
...
void test_stack_push_pop(void) {
    ...
    for (unsigned int i = 0; i < test_N; i++) {
        stack_arr_push(stack, (void *) &i);
    }

    TEST_ASSERT_EQUAL_UINT(test_N, stack->N);
    TEST_ASSERT_EQUAL_UINT(test_N, stack_arr_get_size(stack));


    for (int i = test_N - 1; i >= 0; i--) {
        stack_arr_pop(stack, &result);
        TEST_ASSERT_EQUAL_UINT(i, result);
        TEST_ASSERT_EQUAL_UINT(i, stack_arr_get_size(stack));
    }
    TEST_ASSERT_EQUAL_UINT(0, stack_arr_get_size(stack));

    empty_pop = result;
    stack_arr_pop(stack, &empty_pop);
...
void test_stack_growth_resize(void) {
    ...
    for (unsigned int i = 0; i < 100; i++) {
        ...
        stack_arr_push(stack, &i);
        TEST_ASSERT_EQUAL_UINT(i + 1, stack_arr_get_size(stack));
...
void test_stack_shrink_resize(void) {
    ...
    stack_arr_init(&stack, test_N, sizeof(int));
    TEST_ASSERT_NOT_NULL(stack);
    TEST_ASSERT_NOT_NULL(stack->data);
    for (unsigned int i = 0; i < test_N; i++) {
        stack_arr_push(stack, &i);
